...
// Two components consuming the same hook independently —
// each call is isolated, but both track the same real cursor.
function LiveConsumer({ label, color }: { label: string; color: string }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 13, fontFamily: "'JetBrains Mono','Fira Code',monospace", color: C.text }}>
      ...
      <div style={{ width: "100%", maxWidth: 640 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ color: C.muted, fontSize: 11 }}>useMousePosition.ts</span>
        ...
        <div style={{
          ...
          {active === "problem" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 12, color: C.text, lineHeight: 1.7 }}>
              <div style={{ color: C.muted }}>{"// render props — кожен consumer додає рівень вкладеності"}</div>
              <div style={{ color: C.red }}>{"return ("}</div>
              <div style={{ paddingLeft: 14, color: C.red }}>{"<MouseTracker>"}</div>
              <div style={{ paddingLeft: 28, color: C.orange }}>{"{pos => <div>A: {pos.x}, {pos.y}</div>}"}</div>
              <div style={{ paddingLeft: 14, color: C.red }}>{"</MouseTracker>"}</div>
          ...
          {active === "solution" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 12, color: C.text, lineHeight: 1.7 }}>
              <div style={{ color: C.muted }}>{"// useMousePosition() — 0 рівнів вкладеності"}</div>
              <div>
                <span style={{ color: C.purple }}>const</span> pos = <span style={{ color: C.green }}>useMousePosition</span>();
              </div>
              <div><span style={{ color: C.purple }}>return</span> {"<div>A: {pos.x}, {pos.y}</div>;"}</div>
            </div>
          )}

          {active === "live" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ color: C.muted, fontSize: 11 }}>{"// рухай мишею — обидва виклики читають той самий курсор"}</div>
              <LiveConsumer label="ComponentA" color={C.green} />
              <LiveConsumer label="ComponentB" color={C.accent} />
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          💡 той самий useMousePosition, нуль зайвих компонентів
        </div>
